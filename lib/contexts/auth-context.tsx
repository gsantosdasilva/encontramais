...
type ServiceCategory = 'Diarista' | 'Pintor' | 'Marceneiro' | 'Pedreiro' | 'Eletricista' | 'Encanador';
...
interface AuthContextType {
  ...
  signUp: (email: string, password: string, data: ProfessionalSignUpData) => Promise<string>;
...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  // Registro
  const signUp = async (email: string, password: string, data: ProfessionalSignUpData) => {
    try {
      ...
      // 2. Criar perfil do profissional
      ...
      if (profileError) {
        // Se houver erro ao criar o perfil, tentar deletar o usuário do auth
        await supabase.auth.admin.deleteUser(authData.user.id);
        throw profileError;
      }

      // 3. Informar sobre confirmação de e-mail
